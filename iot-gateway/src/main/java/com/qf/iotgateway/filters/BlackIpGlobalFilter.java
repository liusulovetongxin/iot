...
import com.qf.iotgateway.cache.BlackIpCache;
...
public class BlackIpGlobalFilter implements GlobalFilter, Ordered {
    ...
    @Autowired
    public void setBlackIpCache(BlackIpCache blackIpCache) {
        this.blackIpCache = blackIpCache;
    }
...
//                .flatMap(result->result);
        return Mono.just(blackIpCache.getBlackIpSet())
                .filter(collection-> !collection.contains(exchange.getRequest().getRemoteAddress().getHostString()))
                .map(r-> chain.filter(exchange))
                .defaultIfEmpty(
                        exchange.getResponse().writeAndFlushWith(Mono.just(Mono.just(
                                DefaultDataBufferFactory.sharedInstance.allocateBuffer()
